...
nn.add_layer(3, sigmoid, sigmoid_diff)
nn.add_layer(1, linear, linear_diff)
nn.set_inputnodes(1)
nn.set_cost_function(cost_lin, cost_lin_diff)
nn.compile()
...
X      = np.reshape(np.array([[1.0]]), (1,1))
target = np.reshape(np.array([[2.0]]), (1,1))
...
my_activations = [X] + [nn.act[i](my_zs[i]) for i in range(len(my_zs))]
